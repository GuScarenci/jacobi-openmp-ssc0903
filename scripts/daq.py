    ...
    for _ in range(runs):
        for size in sizes:
            for thread in threads:
                ...
                command = f'make run ARGS="{size} {thread} {0}"'
                ...
                response_parallel = float(match.group(4).split('m')[1].split('s')[0])
                response_times.setdefault((size, thread),[]).append((response_sequential, response_parallel))
...
def interpret_data(reponse_times, show_data:bool=False):
    print("Interpreting data...")
    try:
        data = {}
        for key in raw_data:
            data[key] = {}
            sequential_times = [x[0] for x in raw_data[key]]
            parallel_times = [x[1] for x in raw_data[key]]
            #calculate median, mean and standard deviation
            data[key][sequential] = {}
            data[key][sequential][median] = round(statistics.median(sequential_times),5)
            data[key][sequential][mean] = round(statistics.mean(sequential_times),5)
            data[key][sequential][stdev] = round(statistics.stdev(sequential_times),5)
